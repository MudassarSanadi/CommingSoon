...
import type { TeamMember } from './TeamPage'
...
const ProfilePage: React.FC<ProfilePageProps> = ({ member, setCurrentPage }) => {
  ...
  if (!member) {
    return (
      <>
        <Helmet>
          <title>Profile - Logic Shell</title>
          <meta name="description" content="View Logic Shell team member profiles." />
          <link rel="canonical" href="https://thelogicshell.com/profile" />
        </Helmet>
  ...
  const getAvatarGradient = () => {
    const gradients: Record<string, string> = {
      AN: 'from-blue-600 to-indigo-600',
  ...
  return (
    <>
      <Helmet>
        <title>{member.name} - Logic Shell Team</title>
        <meta name="description" content={`${member.name} - ${member.role} at Logic Shell. ${member.bio}`} />
        <link rel="canonical" href={`https://thelogicshell.com/profile/${member.id}`} />
      </Helmet>
              ...
              <div>
                <div className="mb-8 bg-white rounded-2xl p-6 border border-slate-200 shadow-md">
                  ...
                  <p className="text-slate-600 leading-relaxed">
                    {member.bio} With {member.experience} of experience in {member.role}, {member.name} brings expertise in {member.skills.slice(0, 3).join(', ')}.
                  </p>
                  <div className="mt-4 grid grid-cols-2 gap-3">
                    <div className="bg-slate-50 rounded-lg p-3">
                      <p className="text-xs text-slate-400">Education</p>
                      <p className="text-sm font-medium text-slate-700">{member.education}</p>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-3">
                      <p className="text-xs text-slate-400">Experience</p>
                      <p className="text-sm font-medium text-slate-700">{member.experience}</p>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-3">
                      <p className="text-xs text-slate-400">Location</p>
                      <p className="text-sm font-medium text-slate-700">{member.location}</p>
                    </div>
                    <div className="bg-slate-50 rounded-lg p-3">
